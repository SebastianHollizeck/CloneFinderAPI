...
class MutationFilter:
    ...
    def BranchDecClone(self, seq_list, clone_frequency, Tu2CNV):
        ...
        tree_builder = MegaMP(self.mao_file)
                ...
                for Clo in CloFreDic:  # original hit clo for the tumor
                    ...
                    if CloFreDic[Clo] > 0:
                        ...
                        for Bad in BadPosi2ChnageCloLs:
                            if BadPosi2ChnageCloLs[Bad].count("#" + Clo) != 0 and (
                                CNV[Bad] == "normal" or CNV[Bad] == "Bad-normal"
                            ):
                                Change = "n"
                                for (
                                    Oth
                                    ...
                                    CloFreDic
                                ):  # find multiple mutations at the external branch
                                    if Oth != Clo and CloFreDic[Oth] > 0:
                                        Soth = Clo2Seq["#" + Oth]
                                        if (
                                            Soth[Bad] == "T"
                                            and BadPosi2ChnageCloLs[Bad].count(
                                                "#" + Oth
                                            )
                                            == 0
                                        ):
