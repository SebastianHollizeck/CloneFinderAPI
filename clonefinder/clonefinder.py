...
from decomposition.DecomposedCloneSummarize import DecomposedCloneSummarize
...
tree_builder = MegaMP(maoFile)

# try to read in the ini file
try:

    loader = ParamsLoader(iniFile)
    ...
    # start the summary info
    summary_file = (
        "input data file: "
        + params.input_data_file
        + "\n"
        + "total read count cutoff: "
        + str(params.total_read_cut)
        + "\n"
...
if parser.parse() == False:
    ...
    original_align_builder.initialize(
        tsp_list, True
    )  # True to remove duplicates; False to keep duplicates
    ...
    while RepeatUnion == "y":
        ...
        clusters = SNPClusterGenerator_cnv1(
        ...
        tumor2seqsubs_with_decompose_builder, decomposed_tumor_2clonefrequency, RmCloLs = (
            decomposition.get_decomposed_seq()
        )
        ...
        if RmCloLs == []:
            ...
            summary_file += (
                "hybrid sample genotypes were suggested: " + str(RmCloLs) + "\n"
            )
            ...
            DecomposedCloneSummary = DecomposedCloneSummarize()
            decom_all_seq_builder = DecomposedCloneSummary.add_back_CNVSNV(
                tumor2seqsubs_with_decompose_builder,
                CNV_information,
                seqs_with_ancestor,
                original_clone_freq,
                tsp_list,
            )
            rename_seq_builder, rename_clofre = DecomposedCloneSummary.finalize_results(
                decom_all_seq_builder,
                decomposed_tumor_2clonefrequency,
                original_seq_builder,
                original_clone_freq,
                REP,
            )
            print("fix multiple/backward mutations on decomposed clones")
            rename_seq_builder += ["#hg19", "A" * num_sites]
            no_back_para_mut_decomposed_seq, Tree = DecomposedCloneSummary.fix_back_para_mut_decomposed(
                rename_seq_builder, tree_builder.mao_file
            )
            presence_of_new_decomposed_clone, RmDecom, good_seq_builder = DecomposedCloneSummary.find_decomposed_clone(
                no_back_para_mut_decomposed_seq, REP, Tree
            )
            ...
            no_back_para_mut_decomposed_clone_freq = (
            ...
            no_back_para_mut_decomposed_seq, no_back_para_mut_decomposed_clone_freq = significant_clone.remove_insignificant_clones(
                v_obs,
                clone_frequency_cnv2.Tumor2Clone_frequency,
                clone_frequency_cnv2.hitclone_seq_builder,
                CNV_information_test,
                Significant_cutoff,
            )
            no_back_para_mut_decomposed_seq += ["#hg19", "A" * num_sites]
            print(no_back_para_mut_decomposed_clone_freq)
            Iden = DecomposedCloneSummary.find_new_clone(
                no_back_para_mut_decomposed_seq,
                clone_frequency_cnv.hitclone_seq_builder,
            )
            if Iden == "y":
                presence_of_new_decomposed_clone = "n"
            if presence_of_new_decomposed_clone == "n":
                ...
                if status == True:
                    decomseqs_with_ancestor, decom_tree, decom_nade_map, decom_mask_seq, decom_Good_posi_info = tree_builder.alignment_least_back_parallel_muts(
                        True
                    )  # True will execute code to remove redundant seqs (True is default)
                else:
                    print("failed to run megaMP")
                decomposition_is_better = AnalyzeTree.compare_good_posi_number(
                    Initial_Good_posi_info,
                    decom_Good_posi_info,
                    seqs_with_ancestor,
                    no_back_para_mut_decomposed_seq,
                )
                if decomposition_is_better == "n":
                    print(
                        "decomposed clone genotypes are not good (create more backward/parallel mutations)"
                    )
    ...
    seqs_with_ancestor_newName, CloFre_seqs_with_ancestor_newName, clone_order = OutFile.ReNameCloFreMeg(
    ...
    proportion_of_good_positions = (
        1.0 * number_of_good_positions / len(Initial_Good_posi_info)
    )
    print("proportion of good positions", proportion_of_good_positions)
    summary_file += (
        str(proportion_of_good_positions * 100)
        + "% of SNV sites are not affected by backward/parallel mutations.\n"
    )
    refine_clone = MutationFilter(tumor_seqs, tsp_list, tree_builder.mao_file)
    print("Resolve parallel mutations")
    no_parallel_mutation_seq, no_parallel_mutation_clone_frequency = refine_clone.BranchDecClone(
        seqs_with_ancestor_newName,
        CloFre_seqs_with_ancestor_newName,
        CNV_information_test,
    )
    seqs_with_ancestor_newName, CloFre_seqs_with_ancestor_newName, clone_order = OutFile.ReNameCloFreMeg(
        no_parallel_mutation_seq, no_parallel_mutation_clone_frequency, "number"
    )
    ...
    if status == True:
        ...
        print("failed to run megaMP")
    AA, mask_seq_comb, clone_freq = Align.CombSimClo(
        mask_seq, final_clone_frequency1, 0
    )
    print("test clone hit and remove insignificant clones")
    significant_clone = cluster_test()
    significant_seq, significant_clone_frequency = significant_clone.remove_insignificant_clones(
        v_obs, clone_freq, mask_seq_comb, CNV_information_test, Significant_cutoff
    )
    Align.save_mega_alignment_to_file(
        outFolder + params.input_id + "_CloneFinder.meg", significant_seq
    )
    CloFreAna.save_frequency_table_to_file(
        outFolder + params.input_id + "_CloneFinder.txt",
