class FormatInput:
    def add_low_quality_SNV_info(
        self, CNV_information, total_read, alt_read, total_read_cut, mutant_read_cut
    ):
        New_CNV_information = {}
        for tumor in CNV_information:
            ...
            while c < Len:
                In = CNVinfo[c]
                if CNVinfo[c] == "normal":
                    if TotRead[c] < total_read_cut or (
                        AltRead[c] != 0 and AltRead[c] < mutant_read_cut
                    ):
                        In = "Bad-normal"
                NewIn.append(In)
                c += 1
