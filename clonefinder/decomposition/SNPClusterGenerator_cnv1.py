...
class SNPClusterGenerator_cnv1(Calculation):
    ...
    def convert_Cmatdic_to_mat(self, Cmat_dic):
        Cin = ""
        snv_num = len(Cmat_dic[list(Cmat_dic.keys())[0]])
        clone_order0 = Cmat_dic.keys()
        clone_order = []
        for clone in clone_order0:
            if clone != "hg19":
                clone_order.append("#" + clone)
        snv = 0
        while snv < snv_num:
        ...
        for tumor in self.v_obs:
            ...
            for Posi in NoCNVPosi:
                ...
                for Clone in self.clone_seq:
                    if not Clone in clone_seq_dic_sub:
            ...
            Site2EstSNV_sub = Calculation.compute_estimated_SNVfrequency1(
                self, CloFre, clone_seq_dic_sub
            )

            self.Exp2Diff2ObsSNVID = Calculation.compute_diff1(
                self, ObsSNV_sub, Site2EstSNV_sub
            )

            self.Exp2CluSNV = (
                self.GetExp2CluSNV()
            )  ##get_candidate_decomposed_clones change
            #  print self.Exp2CluSNV
            Clu2Seq, Clu2EstSNVfre = Calculation.GetClu2Seq_cnv(
                self, tumor, NoCNVPosi
            )  #
            ...
            if Clu2Seq != {}:
                ...
                new_Cmat_mat, new_Cmat_dic, new_clone_list = self.add_cluster_Cmat(
                    seq_list_sub
                )
                ...
                hitclone_frequency_dic = clone_frequency_cnv.do_nnls0(
                    new_Cmat_mat, new_clone_list, ObsSNV_sub
                )
                self.tumor2clusters[tumor] = [
                    Clu2EstSNVfre,
                    hitclone_frequency_dic,
                    seq_list_sub,
                ]
