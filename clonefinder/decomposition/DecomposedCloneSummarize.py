...
class DecomposedCloneSummarize:
    def add_back_CNVSNV(
        self,
        DecomTu2Seq_builder_sub,
        CNV_information,
        original_seqs_builder_all,
        original_Tumor2Clone_frequency,
        tsp_list,
    ):
                ...
                for Clo in Clo2Seq:
                    ...
                    while c_all < Len:
                        if CNVinfo[c_all] == "normal":
                            ...
                            if SNVfre_list[c_all] == 0:
                                Seq_all += "A"
                            else:
                                Seq_all += "?"
                        c_all += 1
                    if Clo in Original_clodic_all:
    ...
    def remove_tumor_and_rename_decomposed(
        self,
        tumor2seqs_with_decompose,
        seqs_with_ancestor,
        tumor_seqs,
        REP,
        clone_frequency_for_seqs_with_ancestor,
    ):
        Align = MegaAlignment()
    ...
    def remove_ancestral_decomposed(
        self, remove_tumor_and_rename_decomposed_seq, Error_rate, tumor_seqs
    ):
        #  print 'Tu',tumor_seqs
        Align = MegaAlignment()
            ...
            if name1.find("Clu") != -1:
                ...
                for name2 in SeqOrderIni:
                    if name2 != "#hg19" and name1 != name2:
                        ...
                        if name2.find("Clu") != -1:
                            if Additional_mut_num1 == 0:
                                RmCluClo.append(name1)
                        else:
                            if Der < Error_rate:
                                RmCluClo.append(name1)
        AddedTuLs = []
        ...
        for Tu in TuLs:
            if AddedTuLs.count(Tu) == 0:
                good_seq += [Tu, Tu2Seq[Tu]]
        good_seq += ["#hg19", "A" * len(seq1)]
        return good_seq

    def fix_back_para_mut_decomposed(self, remove_ancestral_decomposed_seq, mao_file):
        tree_builder = MegaMP(mao_file)
        Align = MegaAlignment()
        status = tree_builder.do_mega_mp(
            remove_ancestral_decomposed_seq, "decomposed_seq"
        )
        if status == True:
            decom_seqs_with_ancestor, decom_tree, decom_nade_map, decom_mask_seq, decom_Good_posi_info = tree_builder.alignment_least_back_parallel_muts(
                True
            )  # True will execute code to remove redundant seqs (True is default)
        else:
            print("failed to run megaMP")
        adjusted_decomposed_clone_seq = self.AdjDecClo(
            "Final_NoRedun",
            decom_seqs_with_ancestor,
            decom_nade_map,
    ...
    def AdjDecClo(self, ID, SeqLs, NodeMap, BackFor, Tree):
        ...
        for Clo in Anc2Seq:
            ...
            if Clo.find("#Node") == -1:
                ...
                if BraLen >= 1 and Clo in Clu2Change:
    ...
    def find_decomposed_clone(self, no_back_para_mut_decomposed_seq, REP, Tree):
        ...
        #   print Clo2Seq.keys()
        if RmDecom == []:
            NewClo2Seq_buil = no_back_para_mut_decomposed_seq
        else:
            NewCloDic = {}
            for Clo in Clo2Seq:
                if RmDecom.count(Clo) == 0:
                    NewCloDic[Clo] = Clo2Seq[Clo]
    ...
    def finalize_results(
        self,
        decomposed_seq_builder,
        decomposed_Tumor2Clone_frequency,
        origianl_seq_builder,
        original_Tumor2Clone_frequency,
        REP,
    ):
        Align = MegaAlignment()
        ...
        for Tu in original_Tumor2Clone_frequency:
            if not Tu in decomposed_Tumor2Clone_frequency:
                ...
                for Clo in CloFre0:
                    ...
                    if Fre > 0:
                        if (
                            Clo.find("Clu") != -1 and Clo.find("REP") == -1
                        ) or Clo.find("REP" + str(REP - 1)) != -1:
                            CloFre[Clo + "REP" + str(REP)] = Fre
                            NewCloSeqDic["#" + Clo + "REP" + str(REP)] = DecomSeqDic[
                                "#" + Clo
                            ]
                        else:
                            CloFre[Clo] = Fre
                            if "#" + Clo in OriSeqDic:
                                ...
                                NewCloSeqDic["#" + Clo] = DecomSeqDic["#" + Clo]
            NewCloFre[Tu] = CloFre
