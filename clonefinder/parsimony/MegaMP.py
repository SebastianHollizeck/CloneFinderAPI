...
class MegaMP(object):
    ...
    def _cleanup_temp_files(self):
        try:
            ...
            print("cleanup was not 100% successful")
    ...
    def _get_ancestral_states_files(self):
        ...
        index = 0
        filename = filename = (
            self._temp_dir + self._mega_id + "_ancestral_states_" + str(index) + ".txt"
        )
    ...
    def alignment_least_back_parallel_muts(self, remove_duplicates=True):
        print(
            "finding alignment with least parallel and back mutations..."
        )  # ,self.newick_trees
        files = self._get_ancestral_states_files()
        seq_maker = MakeAncSeqMPMin()

        result = seq_maker.get_best_alignment(
            files, self._mega_id, remove_duplicates, self.newick_trees
        )
        self._cleanup_temp_files()
        return result
